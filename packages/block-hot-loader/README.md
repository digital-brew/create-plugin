...
function enqueue_variations_scripts() {
  // Register variations scripts.
  // Dynamically load dependencies using variations.asset.php generated by
  // @wordpress/dependency-extraction-webpack-plugin.
  $script_asset = require "$plugin_dir_path/build/variations.asset.php";
  
  wp_enqueue_script(
   $slug . '-variations',
   $plugin_dir_url . 'build/variations.js',
   $script_asset['dependencies'],
   $script_asset['version'],
   true
  );
...
```javascript
import { hotVariationLoader, registerVariations } from '@blockhandbook/block-hot-loader';
